...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["free2fa_admin_html"],  # Allow sources
...
class User(BaseModel):
    ...
    otp: Optional[str] = None  # Сделали поле необязательным
    ...
    async with aiosqlite.connect(DATABASE_PATH) as db_connection:
        await db_connection.execute('''
            CREATE TABLE IF NOT EXISTS users (
                ...
                otp_key TEXT DEFAULT '0'
            )
        ''')
        await db_connection.commit()


async def add_otp_key_column():
    async with aiosqlite.connect(DATABASE_PATH) as db_connection:
        # Проверяем, существует ли столбец otp_key в таблице
        cursor = await db_connection.execute("PRAGMA table_info(users)")
        existing_columns = await cursor.fetchall()
        column_names = [column[1] for column in existing_columns]

        if 'otp_key' not in column_names:
            # Если столбец otp_key отсутствует, добавляем его
            await db_connection.execute('''
                ALTER TABLE users ADD COLUMN otp_key TEXT DEFAULT '0'
            ''')
            await db_connection.commit()
...
async def add_user(user: User, _: User = Depends(get_current_user)):
    """
    Adds a new user to the database, including handling the new otp field.

    Args:
        user (User): User object containing user data for insertion, including otp.
    ...
    # Установка значения по умолчанию для otp, если оно не предоставлено
    otp_value = user.otp if user.otp in ["1", "0"] else "0"
    sql_query = (
        'INSERT INTO users (domain_and_username, telegram_id, is_bypass, otp_key) '
        'VALUES (?, ?, ?, ?)'
    )
    async with aiosqlite.connect(DATABASE_PATH) as db_conn:
        try:
            await db_conn.execute(sql_query,
                                  (user.domain_and_username, user.telegram_id, user.is_bypass, otp_value))
...
async def get_all_users():
    ...
    async with aiosqlite.connect(DATABASE_PATH) as db_connection:
        async with db_connection.execute('SELECT * FROM users') as cursor:
            ...
            formatted_users = []
            for user in users:
                # Defining a value for "otp" before creating a dictionary
                if user[3] == "1":
                    otp_status = "Enabled"
                elif user[3] == "0":
                    otp_status = "Disabled"
                ...
                formatted_user = {
                    ...
                    "domain_and_username": str(user[0]),
                    "telegram_id": int(user[1]),  # Conversion to a number
                    # Conversion to a Boolean value
                    "is_bypass": bool(user[2]),
                    "otp": otp_status  # Using a pop-up variable
                }
...
async def update_user(
    ...
    if user_update is None:
        raise HTTPException(
            status_code=400, detail="User update data is required")
    ...
    # Determine which fields need to be updated based on the data provided
    if user_update.domain_and_username:
        update_parts.append("domain_and_username = ?")
        update_values.append(user_update.domain_and_username)
    if user_update.telegram_id is not None:
        update_parts.append("telegram_id = ?")
        update_values.append(user_update.telegram_id)
    if user_update.is_bypass is not None:
        update_parts.append("is_bypass = ?")
        update_values.append(user_update.is_bypass)
    if user_update.otp in ["1", "0"]:
        update_parts.append("otp_key = ?")
        update_values.append(user_update.otp)

    # Add the user name to the end of the list of values for the WHERE clause
    update_values.append(username)
    update_query = f"UPDATE users SET {', '.join(update_parts)} WHERE domain_and_username = ?"

    try:
        async with aiosqlite.connect(DATABASE_PATH) as db_connection:
            await db_connection.execute(update_query, update_values)
            await db_connection.commit()
...
async def get_user(username: str, db_connection=Depends(get_db),
    ...
    async with db_connection.execute(
        ...
        if user:
            return {"domain_and_username": user[0], "telegram_id": user[1], "is_bypass": user[2], "otp_key": user[3]}
