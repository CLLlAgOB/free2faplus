...
async def find_user_by_domain(domain_and_username):
    """Searching for a user in the database. Now also checks if otp_key is neither 0 nor 1."""
    ...
    async with aiosqlite.connect(DATABASE_PATH) as db_connection:
        ...
        query = (
            "SELECT telegram_id, is_bypass, otp_key "
            "FROM users "
        ...
        async with db_connection.execute(query, (domain_and_username,)) as cursor:
            ...
            if result:
                telegram_id, is_bypass, otp_key = result
                # Check that otp_key is neither 0 nor 1
                otp_key_not_configured = otp_key in ['0', '1']
                logger.debug("Found User: %s, tg id: %s, is_bypass: %s, otp_key_not_configured: %s",
                             domain_and_username, telegram_id, is_bypass, otp_key_not_configured)
                return telegram_id, is_bypass, otp_key_not_configured
...
async def get_otp_key_for_user(domain_and_username):
    """Function for getting otp_key from database"""
    async with aiosqlite.connect(DATABASE_PATH) as db_connection:
        query = "SELECT otp_key FROM users WHERE domain_and_username = ?"
        async with db_connection.execute(query, (domain_and_username,)) as cursor:
            result = await cursor.fetchone()
            if result:
                return result[0]
...
async def check_and_verify_otp(request: AuthenticateRequest):
    """Authenticate, waiting for a response."""
    otp = request.otp
    if otp == 0:
        return None
    normalized_username = request.user_name.lower()
    otp_key = await get_otp_key_for_user(normalized_username)

    if not otp_key:
        logger.info("No OTP registered for User %s.", normalized_username)
        return False
    ...
    if otp_valid:
        logger.debug("True OTP from the user: %s", normalized_username)
        return True  # OTP correct

    logger.debug("Invalid OTP from the user: %s", normalized_username)
    return False  # OTP is wrong
...
async def authenticate_user(request: AuthenticateRequest):
    ...
    # OTP verification
    otp_response = await handle_otp_verification(request)
    if otp_response:
        return otp_response

    normalized_username = request.user_name.lower()
    logger.debug("authenticate  User verification: %s",
                 normalized_username)
    telegram_id, is_bypass, otp_key_not_configured = await find_user_by_domain(normalized_username)
    logger.debug("authenticate  Found Telegram ID: %s", telegram_id)

    if telegram_id and telegram_id != 0 and not is_bypass:
        return await handle_auth_with_wait(normalized_username, telegram_id)
    return await handle_auto_reg_or_bypass(normalized_username, telegram_id, is_bypass, otp_key_not_configured)


async def handle_otp_verification(request: AuthenticateRequest):
    """
    Processes the OTP validation, returning the appropriate HTTP response.

    :param request: Authentication request.
    :return: HTTPResponse depending on the result of the OTP check.
    """
    otp_check_result = await check_and_verify_otp(request)
    if otp_check_result is False:
        logger.warning("Invalid OTP for user: %s", request.user_name.lower())
        return response_403()  # Invalid OTP
    if otp_check_result is True:
        logger.info("Valid OTP for user: %s", request.user_name.lower())
        return response_200()  # Valid OTP
    logger.info("No OTP verification result for user: %s",
...
async def handle_auto_reg_or_bypass(normalized_username, telegram_id, is_bypass, otp_key_not_configured):
    """
    ...
    bypass_condition = is_bypass or (
        Config.BYPASS_ENABLED and telegram_id == 0 and otp_key_not_configured)
    if bypass_condition:
    ...
    if not otp_key_not_configured:
        logger.debug("2fa OTP not send or wrong for user %s",
                     normalized_username)
    else:
        logger.warning("2fa is not configured for user %s",
                       normalized_username)
    return response_403()
