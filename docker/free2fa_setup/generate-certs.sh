...
generate_service_cert "free2fa_admin_api" "$CA_EXPIRY_DAYS" "$ADDITIONAL_DNS_NAME_FOR_ADMIN_HTML"
generate_service_cert "free2fa_otp_reg" "$CA_EXPIRY_DAYS" "$ADDITIONAL_DNS_NAME_FOR_ADMIN_HTML"
log "INFO" "Waiting for admin api availability"
until curl -s --cacert /certs/rootca/ca.crt -o /dev/null -w '%{http_code}' https://localhost:8000/health | grep -q "200"; do
    sleep 5
done
