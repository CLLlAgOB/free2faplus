...
if [ -f "$SSSD_CONF" ]; then
    # Ищем строку с domains в файле конфигурации
    DOMAIN_LINE=$(grep "^domains = " "$SSSD_CONF")
    
    # Если строка найдена, извлекаем название домена
    if [ ! -z "$DOMAIN_LINE" ]; then
        DOMAIN=$(echo "$DOMAIN_LINE" | cut -d '=' -f2 | xargs)
    fi
fi

# Если переменная DOMAIN пуста, запросить ввод у пользователя
if [ -z "$DOMAIN" ]; then
...
CONFIG_FILE_SITE_OTP="${ROOT_DIR}sites-available/2fa_otp"
CONFIG_FILE_DICTIONARY="${ROOT_DIR}dictionary"
...
OTP_RADIUS_SECRET=$(generate_random_key)
CA_EXPIRY_DAYS=365
FREE2FA_TELEGRAM_BOT_TOKEN="your-key"
FREE2FA_TELEGRAM_BOT_LANGUAGE=ru
FREE2FA_AUTO_REG_ENABLED="true"
FREE2FA_BYPASS_ENABLED="true"
RADIUS_CLIENT_SECRET="secret123"
RADIUS_PORT_NUMBER=1812
RADIUS_CLIENT_IP="0.0.0.0"
FREE2FA_TIMEOUT=20
RADIUS_START_SERVERS=5
RADIUS_MAX_SERVERS=20
RADIUS_MAX_SPARE_SERVERS=10
RADIUS_MIN_SPARE_SERVERS=5
ADMIN_SECRET_KEY=""
OTP_SESSION_SECRET_KEY=""
ACCESS_TOKEN_EXPIRE_MINUTES=60
RESET_PASSWORD="false"
ALLOW_API_FAILURE_PASS="false"
ADDITIONAL_DNS_NAME_FOR_ADMIN_HTML="free2fa"
MAX_LOGIN_ATTEMPTS=3
BLOCK_DURATION_MINUTES=5
OTP_FIRM_INFO="YourCompany"
OTP_SEPARATOR="::"
RADIUS_SITE_NAME="2fa_default"

# Проверка наличия файла .env и чтение значений переменных
if [ -f "$ENV_FILE" ]; then
    ...
    do
        ...
        if [[ ! $key == \#* ]] && [[ -n $key ]]; then
            # Используем eval для того, чтобы можно было прочитать значение переменной, заданной именем в key
            eval "$key='$value'"
        fi
    done < "$ENV_FILE"
fi
...
RADIUS_PORT_NUMBER=${RADIUS_PORT_NUMBER}
...
OTP_SESSION_SECRET_KEY=${OTP_SESSION_SECRET_KEY:-$(generate_random_key)}
OTP_RADIUS_SECRET=${OTP_RADIUS_SECRET:-$(generate_random_key)}
...
OTP_SEPARATOR=${OTP_SEPARATOR}
RADIUS_SITE_NAME=${RADIUS_SITE_NAME}
EOF
}

# Function to prompt for .env configuration using whiptail
prompt_env_configuration() {
    CA_EXPIRY_DAYS=$(whiptail --inputbox "Enter CA_EXPIRY_DAYS (default 365):" 8 78 $CA_EXPIRY_DAYS --title "CA_EXPIRY_DAYS" 3>&1 1>&2 2>&3)
    check_if_cancel
    FREE2FA_TELEGRAM_BOT_TOKEN=$(whiptail --inputbox "Enter TELEGRAM_BOT_TOKEN (default your-key):" 8 78 $FREE2FA_TELEGRAM_BOT_TOKEN --title "FREE2FA_TELEGRAM_BOT_TOKEN" 3>&1 1>&2 2>&3)
    check_if_cancel
    FREE2FA_TELEGRAM_BOT_LANGUAGE=$(whiptail --inputbox "Enter TELEGRAM_BOT_LANGUAGE (default ru):" 8 78 $FREE2FA_TELEGRAM_BOT_LANGUAGE --title "FREE2FA_TELEGRAM_BOT_LANGUAGE" 3>&1 1>&2 2>&3)
    check_if_cancel
    FREE2FA_AUTO_REG_ENABLED=$(whiptail --inputbox "Enter AUTO_REG_ENABLED (default true):" 8 78 $FREE2FA_AUTO_REG_ENABLED --title "FREE2FA_AUTO_REG_ENABLED" 3>&1 1>&2 2>&3)
    check_if_cancel
    FREE2FA_BYPASS_ENABLED=$(whiptail --inputbox "Enter BYPASS_ENABLED (default true):" 8 78 $FREE2FA_BYPASS_ENABLED --title "FREE2FA_BYPASS_ENABLED" 3>&1 1>&2 2>&3)
    check_if_cancel
    RADIUS_CLIENT_SECRET=$(whiptail --inputbox "Enter RADIUS_CLIENT_SECRET (default secret123):" 8 78 $RADIUS_CLIENT_SECRET --title "RADIUS_CLIENT_SECRET" 3>&1 1>&2 2>&3)
    check_if_cancel
    RADIUS_CLIENT_IP=$(whiptail --inputbox "Enter RADIUS_CLIENT_IP (default 0.0.0.0):" 8 78 $RADIUS_CLIENT_IP --title "RADIUS_CLIENT_IP" 3>&1 1>&2 2>&3)
    check_if_cancel
    FREE2FA_TIMEOUT=$(whiptail --inputbox "Enter TIMEOUT FOR CONFURMATION (default 20):" 8 78 $FREE2FA_TIMEOUT --title "FREE2FA_TIMEOUT" 3>&1 1>&2 2>&3)
    check_if_cancel
    RADIUS_START_SERVERS=$(whiptail --inputbox "Enter RADIUS_START_SERVERS (default 5):" 8 78 $RADIUS_START_SERVERS --title "RADIUS_START_SERVERS" 3>&1 1>&2 2>&3)
    check_if_cancel
    RADIUS_MAX_SERVERS=$(whiptail --inputbox "Enter RADIUS_MAX_SERVERS (default 20):" 8 78 $RADIUS_MAX_SERVERS --title "RADIUS_MAX_SERVERS" 3>&1 1>&2 2>&3)
    check_if_cancel
    RADIUS_MAX_SPARE_SERVERS=$(whiptail --inputbox "Enter RADIUS_MAX_SPARE_SERVERS (default 10):" 8 78 $RADIUS_MAX_SPARE_SERVERS --title "RADIUS_MAX_SPARE_SERVERS" 3>&1 1>&2 2>&3)
    check_if_cancel
    RADIUS_MIN_SPARE_SERVERS=$(whiptail --inputbox "Enter RADIUS_MIN_SPARE_SERVERS (default 5):" 8 78 $RADIUS_MIN_SPARE_SERVERS --title "RADIUS_MIN_SPARE_SERVERS" 3>&1 1>&2 2>&3)
    check_if_cancel
    ADMIN_SECRET_KEY=$(whiptail --inputbox "Enter ADMIN_SECRET_KEY (auto-generated if empty):" 8 78 "$ADMIN_SECRET_KEY" --title "ADMIN_SECRET_KEY" 3>&1 1>&2 2>&3)
    check_if_cancel
    ACCESS_TOKEN_EXPIRE_MINUTES=$(whiptail --inputbox "Enter the session time in the admin portal:" 8 78 $ACCESS_TOKEN_EXPIRE_MINUTES --title "ACCESS_TOKEN_EXPIRE_MINUTES" 3>&1 1>&2 2>&3)
    check_if_cancel
    RESET_PASSWORD=$(whiptail --inputbox "Enter RESET_PASSWORD (default false):" 8 78 $RESET_PASSWORD --title "RESET_PASSWORD" 3>&1 1>&2 2>&3)
    check_if_cancel
    ALLOW_API_FAILURE_PASS=$(whiptail --inputbox "Enter ALLOW_API_FAILURE_PASS (default false):" 8 78 $ALLOW_API_FAILURE_PASS --title "ALLOW_API_FAILURE_PASS" 3>&1 1>&2 2>&3)
    check_if_cancel
    ADDITIONAL_DNS_NAME_FOR_ADMIN_HTML=$(whiptail --inputbox "Enter ADDITIONAL_DNS_NAME_FOR_ADMIN_HTML (default free2fa):" 8 78 $ADDITIONAL_DNS_NAME_FOR_ADMIN_HTML --title "ADDITIONAL_DNS_NAME_FOR_ADMIN_HTML" 3>&1 1>&2 2>&3)
    check_if_cancel
    RADIUS_SITE_NAME=$(whiptail --inputbox "Enter RADIUS SITE NAME (default 2fa_default):" 8 78 $RADIUS_SITE_NAME --title "RADIUS_SITE_NAME" 3>&1 1>&2 2>&3)
    check_if_cancel
    RADIUS_PORT_NUMBER=$(whiptail --inputbox "Enter RADIUS port number (default 1812):" 8 78 $RADIUS_PORT_NUMBER --title "RADIUS_PORT_NUMBER" 3>&1 1>&2 2>&3)
    check_if_cancel
    OTP_SEPARATOR=$(whiptail --inputbox "Enter a separator to separate the password from the OTP (default ::)" 8 78 $OTP_SEPARATOR --title "OTP_SEPARATOR" 3>&1 1>&2 2>&3)
    check_if_cancel
    MAX_LOGIN_ATTEMPTS=$(whiptail --inputbox "Enter a Number of maximum attempts to enter the OTP portal. (default 3)" 8 78 $MAX_LOGIN_ATTEMPTS --title "MAX_LOGIN_ATTEMPTS" 3>&1 1>&2 2>&3)
    check_if_cancel
    BLOCK_DURATION_MINUTES=$(whiptail --inputbox "Enter a Duration in minutes of the ban for exceeding unsuccessful attempts to log in to the OTP portal. (default 5)" 8 78 $BLOCK_DURATION_MINUTES --title "BLOCK_DURATION_MINUTES" 3>&1 1>&2 2>&3)
    check_if_cancel
    OTP_FIRM_INFO=$(whiptail --inputbox "Enter the description that users will see in the OTP application. default (My Company)." 8 78 $OTP_FIRM_INFO --title "OTP_FIRM_INFO" 3>&1 1>&2 2>&3)
    check_if_cancel
    create_env_file
}
...
download_docker_compose_yml() {
    curl -L "https://raw.githubusercontent.com/CLLlAgOB/free2faplus/main/docker-compose/docker-compose.yml" -o docker-compose.yml
...
server __default__ {
    ...
    authorize {
        ...
        if (&User-Password =~ /^(.*)__separator__(\d{6})$/) {
            update request {
                OTP := "%{2}"
                User-Password := "%{1}"
            }
...
sed -i "s/__separator__/$OTP_SEPARATOR/g" "$CONFIG_FILE_SITE"
...
sed "s/__domain__/$shortdomain/g" "$TEMP_FILE" > "$CONFIG_FILE_SITE_OTP"
sed -i "s/__shortname__/$USE_FULLY_QUALIFIED_NAMES/g" "$CONFIG_FILE_SITE_OTP"

# Deleting the temporary file
rm "$TEMP_FILE"

#CONFIG FILE DICTIONARY
echo "ATTRIBUTE       OTP                     3000    integer" > $CONFIG_FILE_DICTIONARY
...
rest {
    ...
    authenticate {
        ...
        method = 'post'
        body = 'json'
        data = '{ "user_name": "%{User-Name}", "client_key": "FREE2FA_API_KEY", "otp": "%{OTP}" }'
        tls = ${..tls}
...
client 2fa_otp {
  ipaddr = 172.254.254.245
  secret = $OTP_RADIUS_SECRET
}
EOF
...
chmod 440 "$CONFIG_FILE_REST" "$CONFIG_FILE_PAM" "$CONFIG_FILE_SITE" "$CHECK_GROUP_SCRIPT" "$CONFIG_FILE_SITE_OTP" "$ENV_FILE"
chown freerad:freerad "$CONFIG_FILE_REST" "$CONFIG_FILE_PAM" "$CONFIG_FILE_SITE" "$CHECK_GROUP_SCRIPT" "$CONFIG_FILE_SITE_OTP"
...
ln -s "$CONFIG_FILE_SITE_OTP" "${ROOT_DIR}sites-enabled/2fa_otp"
